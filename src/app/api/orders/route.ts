...
export async function POST(request: Request) {
  try {
    ...
    const discount = promoCode?.trim().toUpperCase() === "ZIZA" ? subtotal * 0.3 : 0;
    ...
    const orderRows = lines.map((line) => `<tr><td style="padding:8px 0">${line.product!.name} · ${line.product!.format}</td><td style="padding:8px 0;text-align:center">${line.quantity}</td><td style="padding:8px 0;text-align:right">${formatPrice(line.product!.price * line.quantity * (discount ? 0.7 : 1))}</td></tr>`).join("");
