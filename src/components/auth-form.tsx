...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  async function signInWithGoogle() {
    ...
    try { if (!isSupabaseConfigured()) throw new Error("Ajoutez NEXT_PUBLIC_SUPABASE_URL et NEXT_PUBLIC_SUPABASE_ANON_KEY dans .env.local avant d'utiliser Google."); const supabase = createClient(); const { error: googleError } = await supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: `${window.location.origin}/auth/callback` } }); if (googleError) throw googleError; }
    catch (caught) { setError(caught instanceof Error ? caught.message : "La connexion Google est indisponible."); setLoading(false); }
  }
